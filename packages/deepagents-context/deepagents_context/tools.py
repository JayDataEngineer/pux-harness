...
from deepagents_context.store import EventStore, SearchHit, shared_event_store
...
def build_context_tools(
    ...
    *,
    ...
    ``extra_tools`` is an optional list of pre-built tools to append (e.g.
    Docker exec tools injected by the pux adapter). When None, only the
    6 core tools are returned — no Docker dependency.

    Order matters: retrieval first (the high-value surface), indexing next,
    meta last, extras last — so the agent's tool-list presentation foregrounds recall."""
    ...
    return [recall, search, index, stats, doctor, purge] + (extra_tools or [])
